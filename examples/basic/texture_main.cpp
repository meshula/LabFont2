// Simple example of loading and displaying a texture using LabFont abstraction
...
lab_texture logo_texture = nullptr;
...
bool InitWindow() {
    ...
    window = glfwCreateWindow(kWidth, kHeight, "LabFont Texture Example", nullptr, nullptr);
    if (!window) {
...
bool InitLabFont() {
    ...
    // Create the context
    result = lab_create_context(&backend_desc, &context);
...
lab_result Blit() {
    ...
    if (!saved) {
        result = lab_save_render_target(context, render_target, "texture_output.png");
        if (result != LAB_RESULT_OK) {
            std::cerr << "Failed to save render target: " << lab_get_error_string(result);
            // Continue even if saving fails
        } else {
            std::cout << "Render target saved to texture_output.png" << std::endl;
            saved = true;
        }
    }
...
void Cleanup() {
    ...
    if (logo_texture && context) {
        lab_destroy_texture(context, logo_texture);
